AFRAME.registerComponent('loadscene', {
  init: function () {
    ...
    // NOTE: This conditional check works around an A-Frame bug (in 0.8.2/0.9.0) that incorrectly identifies
    // Firefox Reality as a mobile browser (since its `User-Agent` string contains 'Android' and 'Mobile VR').
    // Issue https://github.com/aframevr/aframe/issues/4032 has since been fixed in `master`.
    // Once a new A-Frame version is released, we can replace this with `AFRAME.utils.device.isMobile()`.
